...
import { useArticleContext } from "@/site/hooks/useArticleContext";
...
const LatestArticles = () => {
  ...
  if (data.length > 0 && data[0].articles.length > 0) {
    const allArticles: Article[] = (data[0] as ArticleData).articles.sort((a: Article, b: Article) => {
      const dateA = new Date(a.date).getTime();
      const dateB = new Date(b.date).getTime();
      return dateB - dateA;
    });

    const latestArticle: Article = allArticles[0];

    const remainingArticles: Article[] = allArticles.slice(1);
              ...
              <div className="flex flex-wrap justify-between items-center gap-2">
                <div className="flex flex-wrap gap-2 sm:gap-6">
                  <span className="flex flex-wrap">
                    ...
                    <p>{(data[0] as ArticleData).author}</p>
                  </span>
                  <span className="flex flex-wrap">
                    <p className="font-semibold pr-2">Date</p>
                    <p>{(data[0] as ArticleData).articles[0].date}</p>
                  </span>
                  <span className="flex flex-wrap">
                    <p className="font-semibold pr-2">Read</p>
                    <p>{(data[0] as ArticleData).articles[0].read}</p>
                  </span>
                </div>
                <span className="px-3 py-2 border border-black rounded-full">
                  <p className="uppercase">{(data[0] as ArticleData).articles[0].label}</p>
                </span>
              </div>
            </div>
          </div>
