...
const Articles = () => {
  ...
  const labels: string[] = [
    "All",
    ...new Set(
      data.flatMap((article: ArticleData) => article.articles.map((item: ArticleItem) => item.label))
    ),
  ];

  const filteredArticles = data.flatMap((article: ArticleData) =>
    article.articles
      .filter((item: ArticleItem) =>
  ...
  return (
    <div className="max-w-[95rem] w-full mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border border-black border-collapse mb-48">
        {filteredArticles.map((articleData: ArticleItem & { author: string }, index: number) => (
